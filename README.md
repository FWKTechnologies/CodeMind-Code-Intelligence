...
This is a revised pass over the architecture document, updated against the current state of the system rather than an earlier snapshot of it. The project has gone through a long, unglamorous run of internal revisions — most of them the kind that never make it into a changelog anyone reads, fixing things that only show up once real training or real serving traffic hits them — and this document reflects where that process currently stands, not where it started.

The goal here is depth: not just naming a subsystem, but explaining *why* it exists, *what specifically it does*, and *where it can fail* — so the design can be understood and extended by someone who has never read the source. In line with that goal, this document describes behavior, data flow, and design rationale; it does not reproduce implementation code, exact formulas, or file-level internals. Where a design choice is genuinely load-bearing (a specific ratio, a specific ceiling), it's stated as a number in Section 10 the same way the original architecture already did — that's a statement of *configuration*, not a disclosure of *how it's implemented*.

---

## 0. What this system is — and isn't

Readers of earlier versions of this document tended to walk away with a few specific misreadings. Before anything else, it's worth closing those off directly, because they change how every later section should be read.
...
**Training-time configuration, runtime usage, and distribution are three different regimes, not one.** Section 10's parameter table (hidden dimension, layer counts, batch size, learning rate, PPO buffer size, and so on) describes what the *training run* is configured with — the knobs that shape how the model's weights come to be what they are. Section 9's inference regime (`serve`, `project`, `demo`) is a fixed set of already-trained weights exercised through code paths that never touch the optimizer, the PPO loop, or any training-only bookkeeping. Section 9.2 adds a third regime on top of that: what happens once a trained checkpoint is *packaged for distribution* — which, again, involves none of the training machinery and only a subset of the inference machinery (packaging never runs a forward pass at all). Conflating any two of these is a common and understandable mistake, because they share code — but "runs the same forward pass" is not "is configured the same way," and "reads the same weights" is not "runs the same code path." A production request against a served model never runs an optimizer step; a packaging run never runs a forward pass at all.
...
flowchart TD
    ...
    subgraph Checkpoint["Durability & control"]
        ...
        GUARD{"Preflight + overwrite guard<br/>(Section 9.2)"}
        GATE{"Validation quality<br/>≥ target, or<br/>patience exhausted?"}
        GUARD -.->|blocks a worse<br/>checkpoint from<br/>replacing a better one| CK
        STEP --> CK --> GATE
        GATE -- "no" --> BUF
...
The dataset is swept multiple times rather than once. In the default dual-brain regime this means two full sequential passes — Brain A trains to completion over the full connected dataset first, then Brain B trains to completion over the full dataset again — rather than jointly training both roles on every sample in an interleaved fashion (Section 3.1 explains why the split exists at all). A separate, non-dual-brain training mode instead sweeps the dataset a configurable number of epochs; that mode exists in the codebase as a simpler fallback but isn't the default posture, since the dual-brain split is the design this document is centered on.

Two things make repeated sweeps cheap instead of ruinous regardless of which mode is used: the **graph cache** means CPU-heavy parsing only ever happens once per unique sample across all passes, and, in the dual-brain case specifically, the **phase split** means those two passes aren't "the same thing twice" — Brain A finishes its pass over its own sample schedule before Brain B begins, structured as sequential phases with their own independent sample counters, rather than jointly training both roles on every sample.

Everything in this section — the process pool, the batch buffer, the loss terms, the optimizer step, checkpoint rotation — belongs to the **training regime only**. See Section 9.1 for what's left once training is done and a checkpoint is actually being used, and Section 9.2 for what happens when a finished checkpoint is turned into something meant to be handed to someone else.
...
Generation works over a single, unified representation: a **graph-action codec** that operates on the CPG itself rather than on raw source text. It walks the "generatable" nodes of a target graph in a defined traversal order and emits a sequence of node-type and label actions, in both a standard constrained-generation mode and a variant built specifically for the reinforcement-learning path. A companion renderer turns a decoded action sequence back into source text for whichever language it targets, using per-language-family formatting rules (bracket-style languages versus indentation-style languages, for instance) rather than one universal rendering rule. An earlier design point in this system's history used a second, Python-`ast`-specific codec alongside this one; that codec has since been retired in favor of the single unified graph-action codec described here, which is language-agnostic by construction rather than needing a separate code path per language family (see Section 8).

The decoder that actually produces these action sequences is a causal transformer (9 layers, 16 attention heads) conditioned on the fused brain representation. It supports both a straightforward full-sequence forward pass for computing training loss, and a separate cached-generation path that reuses past key/value tensors across decoding steps rather than recomputing the whole prefix on every new action — the standard technique for making autoregressive generation tractable at scale.

The decoder's position representation is a learned, absolute embedding table (as opposed to a relative scheme like RoPE or ALiBi), which makes context length a genuine design axis rather than something that extrapolates for free: the table's capacity sets a hard ceiling on how long a single generation can run, and how much of that capacity actually receives useful training signal depends on how long the training samples that reach it actually are. Two separate settings exist specifically to keep these concerns apart — the *ceiling* (how long the table is built to support at all) and the *primary training length* (how long most training samples are actually allowed to run before truncation, which governs training cost per step, not the ceiling). A third mechanism, a position-offset augmentation applied during a configurable fraction of training steps, addresses a specific gap that widening the ceiling and the training length alone can't close: if the underlying training corpus rarely contains samples that are naturally long enough to reach the far end of the position table, then simply raising the ceiling doesn't, by itself, guarantee that the table's higher-index rows ever receive gradient — a short sample truncated well before the ceiling never touches those rows regardless of how high the ceiling is set. The augmentation addresses this directly by occasionally shifting a training sample's starting position within the table to a randomly chosen higher offset rather than always starting at position zero, so that shorter samples still exercise the table's higher-index region some of the time, at no additional per-step compute cost (the sequence length being processed doesn't change, only which rows of the table it's mapped onto).

Sampling includes a guard against short, exactly-repeating action cycles (a known degenerate mode of autoregressive generation). By default, generation is **not** capped at a fixed action count — it continues until the model itself emits a stop action — with a very high, fixed internal ceiling that exists purely as a last-resort guard against a genuinely non-terminating generation (a model that never converges toward a stop action and that the repeat-cycle guard doesn't happen to catch, since that guard only catches short, exact repetitions and not slower drift). That internal ceiling is intentionally set far above what any realistic single generation would need, so it is not something a normal use of the system should ever be expected to reach. Separately, and by explicit design, **any lower, practically-meaningful limit on generation length or request duration is not a property of the model or its weights at all** — it's a deployment-time setting, read from a small external configuration file at service start-up, so an operator running this as a hosted API can bound cost and latency per request without needing a different model, a retrained checkpoint, or a code change to do it. A model used directly as a library, rather than through a hosted service, is therefore unbounded by default, in line with the same "the model doesn't decide how it's allowed to be used" separation of concerns.
...
Longer decoder sequences (Section 5) push activation memory per training step up roughly in proportion to sequence length, which is a direct consequence of widening the position-embedding ceiling rather than an incidental cost — gradient checkpointing for the decoder's own transformer blocks trades a further slice of step time for materially lower peak VRAM, applied automatically whenever the same general memory-saving posture is already enabled for the rest of the model, so that widening the decoder's context ceiling doesn't unexpectedly reintroduce the same VRAM pressure the checkpointing was already solving elsewhere.
...
- **The generation decoder** moved from a static-vocabulary token sampler to the graph-action codec described in Section 5 — same architectural slot in the pipeline, completely different notion of what it predicts and how that becomes code. An intermediate design point also maintained a second, Python-specific grammar codec alongside the graph-action one; once the graph-action codec's own traversal-and-render path matured to cover Python as well as every other supported language through the same mechanism, the Python-specific codec had no remaining callers and was removed outright rather than left in place as unreachable code.
...
- **The generation length ceiling** moved from a single fixed, comparatively low action-count cutoff — applied uniformly regardless of who was making the request or what for — to no fixed cutoff by default (Section 5), with a per-deployment configurable limit as the intended mechanism for any operator who needs one. The original fixed ceiling conflated two different concerns that turned out to need different owners: "how long can the model structurally generate for" (a property of the weights and the position table) and "how long should any one request be allowed to run" (an operational, deployment-specific decision) were the same number for a long time, which meant every deployment was stuck with whatever ceiling had been chosen for training, whether or not it fit that deployment's actual needs.
- **Checkpointing** moved from saving model weights only to also persisting the optimizer's own momentum state (without it, every resume effectively restarted Adam's momentum from zero, producing a visible loss spike right after every resume) and adding a graceful-shutdown path so a stop signal triggers an orderly save instead of losing unsaved steps. It has since gained a further safeguard described in Section 9.2: an explicit check, run before any save, that a checkpoint further along in training is never silently replaced by one that's earlier along, rather than trusting that whoever triggers a save always intends to overwrite.
...
Beyond the training loop itself, the same codebase exposes a command-line interface covering training (`train`), a lightweight HTTP serving mode (`serve`) with its own request handler, status reporting (`status`), cache management (`cache`), a scripted demo mode (`demo`), whole-project analysis (`project`) that walks a directory, discovers relevant source files up to a cap, and runs them through the same parser/validator stack used during training, and a distribution-packaging mode (`release`, Section 9.2). "Understand this project," "learn from this sample," and "package this checkpoint for someone else to run" are all backed by the same underlying parsing and model machinery rather than parallel implementations of it. A dedicated large-code handler and multi-project connector extend that same path to inputs that don't fit the single-file/single-sample shape the core training loop otherwise assumes.

The HTTP serving mode publishes its own request/response contract as data — a request for it returns a structured description of every supported operation, its parameters, and, notably, whatever deployment-configured limits (Section 5) are currently in effect for that specific running instance — so a separate frontend, gateway, or client integration can be built against a live, authoritative description of what the service actually accepts rather than a document that can drift out of sync with it.
...
- **Batch size, gradient accumulation, and learning rate (Section 10) have no meaning here.** They shaped how the loaded weights came to be what they are; they say nothing about how many requests `serve` handles at once or how quickly it responds, which are separate, ordinary serving-infrastructure concerns instead — and any request-level ceiling that *does* apply comes from the deployment-time configuration described in Section 5, not from anything trained into the weights.
...
### 9.2 Packaging a checkpoint for distribution

A trained checkpoint and a *distributable* copy of it are treated as two deliberately different artifacts, produced by a dedicated packaging step rather than by simply copying whatever `train` last wrote to disk.

**What gets left out, and why.** A training checkpoint's on-disk footprint is dominated by state that only matters for *continuing* training on the exact same machine and setup: optimizer momentum, which alone is typically the majority of a checkpoint's size and has no role in generating output; and machine-specific paths (where the raw dataset lives, where a local disk cache is mounted) which are meaningless — or actively wrong — on anyone else's machine. The packaging step strips both by default, and, separately, can cast the weights themselves down from training precision to a smaller storage format without a meaningful quality cost for inference, since the compute path during training already runs in reduced precision for most of the forward pass regardless. An explicit opt-in exists for anyone who *does* want a full, continuation-ready checkpoint (including optimizer state, at full precision) rather than an inference-oriented package — packaging doesn't assume one intent is always correct.

**What gets included that a plain checkpoint copy wouldn't have.** The action-codec state the decoder depends on to turn its output back into readable source (Section 5) lives in a separate location from the checkpoint itself during training, purely as an artifact of how the training pipeline is organized — and is trivial to forget when manually assembling something to hand off, since a checkpoint that's missing it still *loads* without error and simply produces unreadable output the first time anyone actually tries to generate something with it. The packaging step folds it in automatically as a normal part of the package, specifically so that failure mode isn't something a recipient can hit at all. Alongside the weights, codec, and stripped configuration, the package also includes generated documentation — a usage guide, a model-card-style summary of architecture and known limitations, and a description of the HTTP contract from Section 9 — assembled from the actual configuration and state of the specific checkpoint being packaged at the moment it's packaged, rather than maintained as separate, hand-written prose that can silently drift out of sync with what the weights actually are. A manifest listing every file in the package alongside a content hash lets a recipient verify nothing was corrupted or truncated in transit.

**What deliberately isn't included.** The package contains a trained model and the minimum supporting material to run and understand it — it does not include the training source code, which is a conscious choice about what's meant to be distributed versus what's meant to stay internal, not an oversight or a limitation of the packaging mechanism.

**Guarding against a specific, easy-to-hit mistake.** Both `train` and `release` run a pre-flight check before doing anything expensive: confirming that a data connection actually exists and isn't empty, that there's enough free disk space for the checkpoints a training run is about to produce, and — the one most worth calling out specifically — that the architecture configuration currently in memory actually matches the checkpoint about to be resumed from. Because certain architecture settings directly determine the shape of the saved tensors, a mismatch here doesn't fail quietly: attempting to load mismatched weights produces an immediate, hard error rather than a silent partial load, but that error surfaces at the moment a checkpoint is loaded — before training has even started — where it's easy to glance past a scary-looking startup warning and not realize the run that follows is starting from scratch rather than resuming, until well after real training time has already been spent on it. The pre-flight check surfaces exactly this condition as a clear, actionable warning up front, specifically to close that gap between "the failure is technically visible" and "the failure is actually going to be noticed before it costs anything." A related, narrower safeguard runs immediately before any checkpoint save completes: if the checkpoint already on disk represents more training progress than what's about to be written, the existing one is preserved under a backup name rather than being silently overwritten by something less trained — protecting against the case where training was resumed without properly loading the prior checkpoint in the first place.

## 10. Key parameters as currently configured

The table below describes **training-time configuration only** — see Section 9.1 for why none of it carries over to how a trained checkpoint behaves once it's actually being used, and Section 9.2 for what happens to these numbers when a checkpoint is packaged for distribution.
...
| Decoder position-table ceiling | 32,768 actions | Hard structural limit on generation length |
| Decoder primary training length | 20,480 actions | Length most training samples are truncated to; governs per-step cost |
| Position-offset augmentation | ~20% of steps train at the full ceiling length; a further ~35% of steps apply a randomized position-table offset | Lets the higher end of the position table receive training signal even from naturally shorter samples (Section 5) |
| Generation length ceiling (library use) | None by default; a fixed, very high internal ceiling exists only as a non-terminating-generation safeguard | Any lower, meaningful limit is a deployment-time setting (Section 5), not a property of the weights |
| Dropout | 0.1 | Applied across brain and decoder |
| PPO replay buffer size | 16 | Policy-gradient stability |
| Total parameters | ≈3.3B | Across both brains, decoder, encoder, and RL/bridge overhead |
| Training epochs (default) | 2 | Applies to the non-dual-brain training mode; the default dual-brain mode instead runs one full pass per brain (Section 1) |
...
**Unlimited-by-default generation length (Section 5) is a usability decision, not a claim of unconditional stability.** Removing a fixed, low generation ceiling makes the system meaningfully more usable for long, single-pass generation tasks, but it also means the responsibility for graceful termination sits more heavily on the model's own learned behavior (emitting a stop action, not drifting into a slow, non-repeating degenerate pattern the cycle guard doesn't catch) than it would under a tight, conservative ceiling. The very high internal safety ceiling exists precisely because that responsibility isn't treated as absolute — but a large gap remains, intentionally, between "the safeguard that prevents true non-termination" and "the length at which output quality is actually still good," and closing that gap is a training-quality question, not something the ceiling itself can answer.
...
- **A single corrupt line never aborts the import** — it's skipped, and that one sample falls through to ordinary on-the-fly parsing later. This applies uniformly to a line that's malformed JSON and to a line that isn't valid text at all (for instance, a line left mid-write when a process was killed) — either case is treated as one skippable bad record rather than being allowed to silently linger as trailing garbage in the file across future runs.
...
Without this subsystem: every sample gets parsed, structurally linked, and validated once per training pass — twice over the life of a default dual-brain run, on hardware billed by the GPU-hour. With it: the identical, deterministic CPU work happens once, ever, per unique sample — either offline ahead of time or on first live encounter during training, whichever comes first — and every subsequent encounter, in that run or a future one, is a cache lookup instead of a recomputation.
